...
.directive('flowChart', function() {
  return {
  	restrict: 'E',
  	templateUrl: "views/partials/flowchart.html",
  	replace: true,
  	scope: {
  		chart: "=chart",
  	},
...
.controller('FlowChartController', ['$scope', 'dragging', '$element', function FlowChartController ($scope, dragging, $element) {
  ...
  $scope.templateForCustomShape = function (cs) {
    
  
    return "<circle r=\"20\" cx=\"10\" ng-attr-cy=\"{{10 + 10}}\" fill=\"black\"></circle>";
  };
	...
	$scope.sizeAttributes = $scope.$root.sizeAttributes;
}]);
