...
function WorkflowsAPI (
  ...
  eventBus.on('workflow-output', function (boxName, workflowId, output) {
...
WorkflowsAPI.prototype.getWorkflowEnd = function (boxName, id) {
  var self = this;
  ...
  return function () {
    ...
    var graphId = self.notRunning[id].graphId;
          ...
          noflo.createNetwork(g, function (n) {
            ...
            n.connect(function () {
              ...
              n.on('end', function () {
                ...
                self.getWorkflowEnd(boxName, n.id, doc.name)
              });

              for (var c in n.processes) {
                n.processes[c].component.api = self.api;
...
WorkflowsAPI.prototype.listGraphs = function (boxName, callback) {
  ...
  self.collections.find(boxName, '_graphs', {}, callback);
};
